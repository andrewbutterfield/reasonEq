...
%\includegraphics[scale=0.75]{doc/images/n-x}
% <img src="images/2-equiv-id-one-a1.png" alt="Proof Start" width=40%>

% We see that the goal has changed,and also that we have the start of a proof transcript showing the the original goal was transformed by a match with the `$\equiv$_assoc` law at the top-level.

% ### Proof Step 2

% We now want to focus attention on the `Q$\equiv$Q` sub-part of the goal. It is the second argument to thew top-level `$\equiv$` operator, so we want to move the focus down to that 2nd argument.
...
% ```
% Matches:
% 1 : “$\equiv$_refl” gives     true  
% 2 : “$\equiv$_symm” gives     Q$\equiv$Q  
% 3 : “$\equiv$_symm” gives     Q$\equiv$Q  
% 4 : “$\equiv$_symm” gives     (Q$\equiv$Q)$\equiv$Q$\equiv$Q  
% 5 : “$\equiv$_symm” gives     P$\equiv$(Q$\equiv$Q)$\equiv$P  
% 6 : “$\equiv$_symm” gives     P$\equiv$(Q$\equiv$Q)$\equiv$P  
% 7 : “$\equiv$_symm” gives     Q$\equiv$Q$\equiv$(Q$\equiv$Q)  
% 8 : “∨_idem” gives     (Q$\equiv$Q)∨(Q$\equiv$Q)  
% 9 : “⟹ _def” gives     P⟹ (Q$\equiv$Q)$\equiv$P∨(Q$\equiv$Q) 
% ```
...
% Note that we are still focussed at the same place. Here we matched the law `$\equiv$_refl` at the second component of the top-level goal (`@[2]`).
